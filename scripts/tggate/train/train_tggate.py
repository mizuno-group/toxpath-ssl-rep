...
parser.add_argument('--batch_folder', type=str, default="/work/gd43/share/tggates/liver/batchext")
...
class Dataset_Batch(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if type(transform)!=list:
            ...
            self._transform = transform
        # load data
        with open(f"{args.batch_folder}/fold{fold}_{batch}.npy", 'rb') as f:
            self.data = np.load(f)
...
def main(resume=False):
    ...
    if resume:
        ...
        model, criterion, optimizer, scheduler, early_stopping = prepare_model_train(
            model_name=args.model_name, ssl_name=args.ssl_name,
            patience=args.patience, delta=args.delta, lr=args.lr, num_epoch=args.num_epoch,
            DEVICE=DEVICE,
        )
